...
class contentboxaudio extends Module
{
	...
	function __construct()
	{
		...
		$this->displayName = $this->l('contentBox-Audio');
		$this->description = $this->l('Bloc avec le contenu audio');
	...
	function hookdisplayRightColumn($params)
	{
		...
		if (file_exists(dirname(__FILE__).'/content.xml'))
		{
			...
			if ($xml = simplexml_load_file(dirname(__FILE__).'/content.xml'))
			{
				...
				return $this->display(__FILE__, 'contentboxaudio.tpl');
			}
		}
		return false;
	}
